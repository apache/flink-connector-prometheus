...
public class AmazonManagedPrometheusWriteRequestSigner implements PrometheusRequestSigner {
     ...
     *
     * @param requestHeaders original Http request headers. It must be mutable. For efficiency, any new header is
     *                       added to the map, instead of making a copy.
    ...
    public void addSignatureHeaders(Map<String, String> requestHeaders, byte[] requestBody) {
        ...
        requestHeaders.put("x-amz-content-sha256", contentHashString); // this header must be included before generating the Authorization header
        ...
        String sessionToken = (awsCreds instanceof AWSSessionCredentials) ? ((AWSSessionCredentials) awsCreds).getSessionToken() : null;

        AWS4SignerForAuthorizationHeader signer = new AWS4SignerForAuthorizationHeader(remoteWriteUrl, "POST", "aps", awsRegion);
        // computeSignature also adds 'Host', 'X-Amz-Date' and 'x-amz-security-token' to the requestHeaders Map
        String authorization = signer.computeSignature(requestHeaders,
