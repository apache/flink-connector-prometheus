...
public class PrometheusSinkWriter extends AsyncSinkWriter<PrometheusTimeSeries, Types.TimeSeries> {
    ...
    /***
     Batching of this sink is in terms of Samples, not bytes. The goal is adaptively increase the number of
     Samples in each batch, a WriteRequest sent to Prometheus, to a configurable number. This is the parameter
     maxBatchSizeInBytes.

     getSizeInBytes(requestEntry) returns the number of Samples (not bytes) and maxBatchSizeInBytes is actually in
     terms of Samples (not bytes).

     In AsyncSinkWriter, maxBatchSize is in terms of requestEntries (TimeSeries). But because each TimeSeries
     contains 1+ Samples, we set maxBatchSize = maxBatchSizeInBytes.

     maxRecordSizeInBytes is also calculated in the same unit assumed by getSizeInBytes(..). In our case is the
     max number of Samples in a single TimeSeries sent to the Sink.
     We are limiting the number of Samples in each TimeSeries to the max batch size, setting
    ...
    public PrometheusSinkWriter(
        ...
        super(
                ...
                states);
        this.requestBuilder = new PrometheusRemoteWriteHttpRequestBuilder(prometheusRemoteWriteUrl, requestSigner);
        this.asyncHttpClient = asyncHttpClient;
        this.counters = counters;
    }

    /**
     * This is the "size" of the request entry (a {@link Types.TimeSeries time-series}) used for batching.
     * Regardless the name of the method, it returns the number of {@link Types.Sample samples} in the time-series
     * (not bytes), to support batching in terms of samples.
     *
    ...
    @Override
    protected void submitRequestEntries(List<Types.TimeSeries> requestEntries, Consumer<List<Types.TimeSeries>> requestResult) {
        int timeSeriesCount = requestEntries.size();
        long sampleCount = RequestEntrySizeUtils.countSamples(requestEntries);
        if (LOG.isTraceEnabled()) {
            LOG.trace("Writing {} time-series containing {} samples ", timeSeriesCount, sampleCount);
        }
        ...
        asyncHttpClient.execute(postRequest, new ResponseCallback(timeSeriesCount, sampleCount, counters, requestResult));
    }
    ...
    static class ResponseCallback implements FutureCallback<SimpleHttpResponse> {
        ...
        public ResponseCallback(int timeSeriesCount, long sampleCount, SinkCounters counters, Consumer<List<Types.TimeSeries>> requestResult) {
        ...
        public void completed(SimpleHttpResponse response) {
            if (RemoteWriteResponseClassifier.isSuccessResponse(response)) {
                LOG.debug("{},{} - successfully posted {} time-series, containing {} samples", response.getCode(), response.getReasonPhrase(), timeSeriesCount, sampleCount);
                ...
                if (RemoteWriteResponseClassifier.isNonRetriableErrorResponse(response)) {
                    LOG.warn("{},{} {} (discarded {} time-series, containing {} samples)", response.getCode(), response.getReasonPhrase(), responseBody, timeSeriesCount, sampleCount);
                    ...
                    LOG.warn("{},{} {} (after retry limit reached, discarded {} time-series, containing {} samples)", response.getCode(), response.getReasonPhrase(), responseBody, timeSeriesCount, sampleCount);
        ...
        public void failed(Exception ex) {
            LOG.warn("Exception executing the remote-write (discarded {} time-series containing {} samples)", timeSeriesCount, sampleCount, ex);
            counters.inc(NUM_SAMPLES_DROPPED, sampleCount);
            counters.inc(NUM_WRITE_REQUESTS_PERMANENTLY_FAILED);
        }
        ...
        public void cancelled() {
            LOG.warn("Write request execution cancelled (discarded {} time-series containing {} samples)", timeSeriesCount, sampleCount);
            counters.inc(NUM_SAMPLES_DROPPED, sampleCount);
            counters.inc(NUM_WRITE_REQUESTS_PERMANENTLY_FAILED);
        }
    }
