...
public class PrometheusSinkBuilder extends AsyncSinkBaseBuilder<PrometheusTimeSeries, Types.TimeSeries, PrometheusSinkBuilder> {
    ...
    private static final int DEFAULT_MAX_BUFFERED_REQUESTS = 1000; // Max nr of requestEntry that will be buffered
    ...
    public AsyncSinkBase<PrometheusTimeSeries, Types.TimeSeries> build() {
        ...
        int actualMaxBatchSizeInSamples = Optional.ofNullable(getMaxBatchSizeInSamples()).orElse(DEFAULT_MAX_BATCH_SIZE_IN_SAMPLES);
        int actualMaxBufferedRequests = Optional.ofNullable(getMaxBufferedRequests()).orElse(DEFAULT_MAX_BUFFERED_REQUESTS);
        long actualMaxTimeInBufferMS = Optional.ofNullable(getMaxTimeInBufferMS()).orElse(DEFAULT_MAX_TIME_IN_BUFFER_MS);
        int actualMaxRecordSizeInSamples = Optional.ofNullable(getMaxRecordSizeInSamples()).orElse(getMaxBatchSizeInSamples()); // By default, set max record size to max batch size (in samples)

        int actualSocketTimeoutMs = Optional.ofNullable(getSocketTimeoutMs()).orElse(PrometheusAsyncHttpClientBuilder.DEFAULT_SOCKET_TIMEOUT_MS);

        Preconditions.checkArgument(StringUtils.isNotBlank(prometheusRemoteWriteUrl), "Missing or blank Prometheus Remote-Write URL");
        ...
        Preconditions.checkArgument(actualMaxBatchSizeInSamples > 0, "Max batch size (in samples) must be positive");
        Preconditions.checkArgument(actualMaxRecordSizeInSamples <= actualMaxBatchSizeInSamples, "Max record size (in samples) must be <= Max batch size");
        ...
        LOG.info("Prometheus sink configuration:" +
                ...
                retryConfiguration.getInitialRetryDelayMS(), retryConfiguration.getMaxRetryDelayMS(), retryConfiguration.getMaxRetryCount(),
                socketTimeoutMs);
    ...
    public PrometheusSinkBuilder setMaxBatchSizeInBytes(long maxBatchSizeInBytes) {
        throw new UnsupportedOperationException("maxBatchSizeInBytes is not supported by this sink");
    }

    @Override
    public PrometheusSinkBuilder setMaxRecordSizeInBytes(long maxRecordSizeInBytes) {
        throw new UnsupportedOperationException("maxRecordSizeInBytes is not supported by this sink");
    }
