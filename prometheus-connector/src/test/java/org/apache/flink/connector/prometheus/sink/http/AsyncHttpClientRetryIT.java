...
public class AsyncHttpClientRetryIT {
    ...
    @Test
    public void shouldRetryOn500UpToRetryLimitThenSuccessfullyReturn(WireMockRuntimeInfo wmRuntimeInfo) throws URISyntaxException, IOException {
        ...
        PrometheusAsyncHttpClientBuilder clientBuilder = new PrometheusAsyncHttpClientBuilder(WireMockTestUtils.retryConfiguration(retryLimit));
        try (CloseableHttpAsyncClient client = clientBuilder.buildAndStartClient(counters)) {
            ...
            await().untilAsserted(() -> {
                verify(exactly(expectedRequestCount), postRequestedFor(urlEqualTo("/remote_write")));
            });
        }
    }

    @Test
    public void shouldRetryOn429UpToRetryLimitThenSuccessfullyReturn(WireMockRuntimeInfo wmRuntimeInfo) throws URISyntaxException, IOException {
    ...
    @Test
    public void shouldNotRetryOn404ThenSuccessfullyReturn(WireMockRuntimeInfo wmRuntimeInfo) throws URISyntaxException, IOException {
        stubFor(post("/remote_write").willReturn(notFound()));
        var counters = mock(SinkCounters.class);

        PrometheusAsyncHttpClientBuilder clientBuilder = new PrometheusAsyncHttpClientBuilder(WireMockTestUtils.retryConfiguration(2));
    ...
    @Test
    void shouldNotRetryOn200OkThenSuccessfullyReturn(WireMockRuntimeInfo wmRuntimeInfo) throws URISyntaxException, IOException {
