...
class SinkWriterErrorHandlingBehaviorConfigurationTest {
    ...
    private static final SinkWriterErrorHandlingBehaviorConfiguration DEFAULT_CONFIG =
            SinkWriterErrorHandlingBehaviorConfiguration.builder().build();

    @Test
    public void shouldDefaultToFailOnMaxRetryExceeded() {
        assertEquals(OnErrorBehavior.FAIL, DEFAULT_CONFIG.getOnMaxRetryExceeded());
    }

    @Test
    public void shouldDefaultToFailOnHttpClientIOFail() {
        assertEquals(OnErrorBehavior.FAIL, DEFAULT_CONFIG.getOnHttpClientIOFail());
    }

    @Test
    public void shouldDefaultToFailOnPrometheusNonRetriableError() {
        assertEquals(OnErrorBehavior.FAIL, DEFAULT_CONFIG.getOnPrometheusNonRetriableError());
    }
}
